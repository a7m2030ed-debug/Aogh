...
**كيف يصل التحديث للمستخدم؟** على آبل: يمرّ بالمراجعة، ثم يُثبَّت **تلقائياً
في الخلفية** عند أغلب المستخدمين لأن التحديث التلقائي مفعّل افتراضياً. ومن
أطفأه يرى شارة في المتجر — أو لا يحدّث أبداً. **آبل لا توفّر أي وسيلة لإجبار
أحد**، فالإجبار يُبنى داخل التطبيق.

### ما يتغيّر بلا إصدار جديد

| البيانات | كيف تصل |
|---|---|
| المباريات والأخبار | من الشبكة مباشرة — تتغيّر فوراً |
| **قائمة القنوات** | من [أصول إصدار `data`](https://github.com/a7m2030ed-debug/Aogh/releases/tag/data) — يقرؤها التطبيق عند التشغيل ويخزّنها ست ساعات |
| **بوّابة التحديث** | من الأصول نفسها |
| **رابط وسيط المباريات** | من الأصول نفسها — تشغيله وإطفاؤه بتحرير سطر واحد |

الفحص الأسبوعي للقنوات ينشرها تلقائياً، فموت رابط أو إضافة قناة يصل كل
المستخدمين خلال دقائق **بلا إصدار ولا مراجعة**. والنسخة المرفقة تبقى شبكة
الأمان: بلا شبكة أو بمصدر معطوب يرى المستخدم ما بُني مع التطبيق لا شاشة فارغة.

### بوّابة التحديث

يُحرَّر [`Tools/app-version.json`](Tools/app-version.json) ويُدفع، فيُنشر خلال
دقيقة:

```json
{ "latest": "1.1", "minimum": "0.0", "ios": "https://apps.apple.com/app/id…" }
```

* **`latest`** أعلى من المثبَّت ← شريط لطيف أعلى الشاشة: «تحديث» و«لاحقاً».
  «لاحقاً» يطويه لهذا الإصدار فقط، فالتالي يُعلن عن نفسه.
* **`minimum`** أعلى من المثبَّت ← شاشة حاجبة لا تُطوى. **لعطل حقيقي فقط**،
  واتركه على `0.0` في الأحوال العادية.
* `titleAr`/`notesAr` (وما يقابلهما بالإنجليزية) تكتب نصّ الشريط بلغة المستخدم.

بعد نشر التطبيق ضع رابطه الحقيقي في `ios` — الرابط الافتراضي فيه معرّف وهمي.

### النشر التدريجي

فعّل **Phased Release** في App Store Connect مع كل تحديث: يوزّعه على سبعة أيام
(١٪ ← ١٠٠٪) ويمكن إيقافه فوراً إن ظهر خلل، بدل أن يصل الجميع دفعةً واحدة.
...
ثلاثة مسارات بالترتيب: **مفتاح المستخدم الشخصي** إن وضعه، ثم **[الوسيط
المنشور](../Proxy/README.md)** إن كان مفعّلاً — وهو ما يراه أغلب الناس لأنه لا
يحتاج منهم تسجيلاً — ثم **المصدر القديم** بحدوده المعروفة. وكلٌّ يسقط إلى ما
بعده، فلا يبقى أحد بشاشة فارغة.

> **قبل النشر للناس:** بلا وسيط يحتاج كل مستخدم مفتاحاً باسمه، وأغلب من يحمّل
> تطبيقاً لا يسجّل في خدمة ليستعمله. [شغّل الوسيط](../Proxy/README.md) — كله من
> المتصفّح، ومجاني على Cloudflare.
