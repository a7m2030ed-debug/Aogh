...
أو — لمن لا يريد سطر أوامر — بالنقر على `تشغيل-ويندوز.bat` (أو `bash تشغيل-ماك-لينكس.sh`) في مجلد `staff`، وهو يُنشئ `.env` بوضع الشبكة المحلية ويفتح المتصفح.
...
| `PUBLIC_URL` | http://localhost:PORT | العنوان العام. **يجب أن يكون https في الإنتاج** ما لم يكن `LAN=1` |
| `LAN` | 0 | `1` لوضع الشبكة المحلية: يُسمح بـhttp داخل شبكة المجمع، ويُطبع تنبيه عند كل تشغيل |
| `TLS_CERT` · `TLS_KEY` | — | شهادة وملف مفتاح (ولو محلية موقَّعة ذاتيًا) فيعمل النظام على https مباشرة بلا Nginx |
